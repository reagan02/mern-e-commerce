	...
	const deleteCart = async () => {
		...
		for (let i = 0; i < checkedCartItems.length; i++) {
			if (checkedCartItems[i]) {
				...
				try {
					await axios.delete(
						`https://exclusive-mern.vercel.app/api/cart/${productID}?userID=${userID}`
					);
					setCart((prev) => {
						return {
							...prev,
				...
				if (cartData.products) {
					...
					const stocksPromise = Promise.all(
						cartData.products.map(async (product) => {
							const response = await axios.get(
								`https://exclusive-mern.vercel.app/api/cart/products/${product.productID}/variants/${product.variantIndex}`
							);
